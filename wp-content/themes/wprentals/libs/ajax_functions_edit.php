...
if( !function_exists('wpestate_ajax_add_allinone_custom') ):
    function wpestate_ajax_add_allinone_custom(){
      ...
      //  check_ajax_referer( 'booking_ajax_nonce','security');
        ...
        $mega_details_array = get_post_meta($property_id, 'mega_details'.$property_id,true );
        ...
        update_post_meta($property_id, 'mega_details'.$property_id,$mega_details_array );
...
if( !function_exists('wpestate_ajax_delete_custom_period') ):
    function wpestate_ajax_delete_custom_period(){ 
        ...
        $mega_details_array = get_post_meta($edit_id, 'mega_details'.$edit_id,true );
        ...
        update_post_meta($edit_id, 'custom_price'.$edit_id,$price_array );
        update_post_meta($edit_id, 'mega_details'.$edit_id,$mega_details_array );
        print 'deleted';
        die();
    }
endif;    
...
if( !function_exists('wpestate_ajax_front_end_submit') ):
    ...
    if ( $paid_submission_status!='membership' || ( $paid_submission_status== 'membership' || wpestate_get_current_user_listings($userID) > 0)  ){ // if user can submit        
        ...
        if($post_id) {
            ...
            $message .= sprintf( esc_html__( "A user has submited a new listing on  %s! You should go check it out. This is the listing title: %s",'wpestate'), get_option('blogname'),$submit_title) . "\r\n\r\n";
 
            wp_mail(get_option('admin_email'),
		    sprintf(esc_html__( '[%s] New Listing Submission','wpestate'), get_option('blogname')),
                    $message,
                    $headers);
        ...
        if( isset( $_POST['listing_edit'] ) ) {
            if( !is_numeric($_POST['listing_edit'] ) ){
                ...
                if( $current_user->ID != $the_post->post_author ) {
                    ...
                    $allowed_html    =   array();
                    $cleaning_fee    =   intval( $_POST['cleaning_fee']);
                    $city_fee        =   intval( $_POST['city_fee']);
                    $price           =   intval( $_POST['price']);
                    $price_week      =   intval( $_POST['price_week']);
                    $price_month     =   intval( $_POST['price_month']);

                    $cleaning_fee_per_day           =   intval( $_POST['cleaning_fee_per_day']);
                    $city_fee_per_day               =   intval( $_POST['city_fee_per_day']);
                    $min_days_booking               =   intval( $_POST['min_days_booking']);
                    $price_per_guest_from_one       =   intval( $_POST['price_per_guest_from_one']);
                    $price_per_weekeend             =   intval( $_POST['price_per_weekeend']);
                    $checkin_change_over            =   intval( $_POST['checkin_change_over']);
                    $checkin_checkout_change_over   =   intval( $_POST['checkin_checkout_change_over']);
                    $extra_price_per_guest          =   intval( $_POST['extra_price_per_guest']);
                    $overload_guest                 =   intval( $_POST['overload_guest']);
