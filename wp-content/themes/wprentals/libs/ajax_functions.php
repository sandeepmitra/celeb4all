...
if( !function_exists('wpestate_wp_new_user_notification') ):
    ...
    function wpestate_wp_new_user_notification( $user_id, $plaintext_pass = '' ) {
        ...
        $message  = sprintf( esc_html__( 'New user registration on %s:','wpestate'), get_option('blogname') ) . "\r\n\r\n";

        $message .= sprintf( esc_html__( 'Username: %s','wpestate'), $user_login ) . "\r\n\r\n";

        $message .= sprintf( esc_html__( 'E-mail: %s','wpestate'), $user_email ) . "\r\n";
        ...
        @wp_mail(
                ...
                sprintf(esc_html__( '[%s] New User Registration','wpestate'), get_option('blogname') ),
        ...
        $message .= sprintf( esc_html__( "Welcome to %s! You can login now using the below credentials: ",'wpestate'), get_option('blogname')) . "\r\n\r\n";

        $message .= sprintf( esc_html__( 'Username: %s','wpestate'), $user_login ) . "\r\n";

        $message .= sprintf( esc_html__( 'Password: %s','wpestate'), $plaintext_pass ) . "\r\n\r\n";

        $message .= sprintf( esc_html__( 'If you have any problems, please contact me at %s.','wpestate'), get_option('admin_email') ) . "\r\n\r\n";

        $message .= esc_html__( 'Thank you!','wpestate');
        ...
        wp_mail(
                ...
                sprintf( esc_html__( '[%s] Your username and password','wpestate'), get_option('blogname') ),
        ...
        */
        ...
        if(isset($_POST['tipul'])){
            ...
            if($tipul==1){
                ...
                if( !estate_verify_onetime_nonce($_POST['security-register'], 'register_ajax_nonce_wd') ){
        ...
        if ( !$user_id and email_exists($user_email) == false ) {
            ...
            $user_id         = wp_create_user( $user_name, $random_password, $user_email, array('role' => 'celleb') );
            ...
            if ( is_wp_error($user_id) ){
                ...
                if ( $acc_type == '0' ) {
           wp_update_user( array( 'role' => 'celleb' ) );
        } else {
            wp_update_user( array( 'role' => 'celleb' ) );
        }
            }
...
if( !function_exists('wpestate_ajax_forgot_pass') ):
    ...
    function wpestate_ajax_forgot_pass(){
        ...
        $message = esc_html__( 'Someone requested that the password be reset for the following account:','wpestate') . "\r\n\r\n";

        $message .= get_option('siteurl') . "\r\n\r\n";

        $message .= sprintf(esc_html__( 'Username: %s','wpestate'), $user_login) . "\r\n\r\n";

        $message .= esc_html__( 'If this was a mistake, just ignore this email and nothing will happen.','wpestate') . "\r\n\r\n";

        $message .= esc_html__( 'To reset your password, visit the following address:','wpestate') . "\r\n\r\n";

        $message .= wpestate_tg_validate_url($post_id,$type) . "action=reset_pwd&key=$key&login=" . rawurlencode($user_login) . "\r\n";

        if ( $message && !wp_mail($user_email, esc_html__( 'Password Reset Request','wpestate'), $message,  $headers) ) {

                echo "<div class='error'>".esc_html__( 'Email failed to send for some unknown reason.','wpestate')."</div>";
                      ...
                      <div class="modal-header"> 
                       ...
                       <div class="modal-body">
                        ...
                        <div id="ajax_login_div" class="'.$show_login.'">
                            ...
                            '. do_shortcode( "[ultimatemember form_id=397]" ) .'
                               ...
                               <!-- <div class="loginrow">
                                ...
                                <!-- <button id="wp-login-but" class="wpb_button  wpb_btn-info  wpb_regularsize   wpestate_vc_button  vc_button" data-mixval="'.$propid.'">'.esc_html__( 'Login','wpestate').'</button> -->
                                    ...
                                    <div class="login-links" >';
                                        ...
                                        //[register_form type=""][/register_form]
                        ...
                        <li role="presentation" class="active"><a href="#celleb_signup" aria-controls="home" role="tab" data-toggle="tab">Cellebrity Signup</a></li>
                        <li role="presentation"><a href="#user_signup" aria-controls="profile" role="tab" data-toggle="tab">User Signup</a></li>
                      </ul>

                        <div class="tab-content">
                            <div role="tabpanel" class="tab-pane active" id="celleb_signup">' . do_shortcode('[ultimatemember form_id=396]') . '</div>
                            <div role="tabpanel" class="tab-pane" id="user_signup"> ' . do_shortcode( '[ultimatemember form_id=423]' ) . ' </div>
                        </div>
                                ...
                                <div id="reveal_login"><a href="#">'.esc_html__( '','').'</div> ';
...
if( !function_exists('wpestate_ajax_booking_pay') ):
    ...
    function wpestate_ajax_booking_pay(){ 
        ...
        $total_price        =   round( ($total_price/100*$book_down), 2);
    ...
    function wpestate_ajax_resend_for_approval(){ 
        ...
        if($free_list>0 ||  $free_list==-1){
            ...
            if(  $new_status== 'publish'){
                ...
                $message       .=   sprintf( esc_html__( "A user has re-submited a new property on  %s! You should go check it out.This is the property title: %s",'wpestate'), get_option('blogname'),$submit_title) . "\r\n\r\n";



                wp_mail(get_option('admin_email'),

                sprintf(esc_html__( '[%s] Expired Listing sent for approval','wpestate'), get_option('blogname')),
