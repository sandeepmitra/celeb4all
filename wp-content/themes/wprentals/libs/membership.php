...
if( !function_exists('membership_package') ):
    function membership_package( $post ) {
	    ...
	    global $post;
                ...
                <p class="meta-options">
                    <label for="pack_price">'.esc_html__( 'Package Price in ','wpestate'). ' ' .get_option('wp_estate_submission_curency').'</label><br />
...
if( !function_exists('wpestate_downgrade_to_pack') ):
    function wpestate_downgrade_to_pack( $user_id, $pack_id ){
        ...
        $message  = esc_html__( 'Account Downgraded,','wpestate') . "\r\n\r\n";
        ...
        wp_mail($user_email,
                sprintf(esc_html__( '[%s] Account Downgraded','wpestate'), get_option('blogname')),
                $message,
                $headers);
...
if( !function_exists('wpestate_downgrade_to_free') ): 
    function wpestate_downgrade_to_free($user_id){
        ...
        $headers = 'From: No Reply <noreply@'.$_SERVER['HTTP_HOST'].'>' . "\r\n";
        $message  = esc_html__( 'Membership Cancelled,','wpestate') . "\r\n\r\n";
        $message .= sprintf( esc_html__( "Your subscription on %s was cancelled because it expired or the recurring payment from Paypal was not processed. All your listings are no longer visible for our visitors but remain in your account. Thank you. ",'wpestate'), get_option('blogname')) . "\r\n\r\n";

        wp_mail($user_email,
                sprintf(esc_html__( '[%s] Membership Cancelled','wpestate'), get_option('blogname')),
                $message,
                $headers);
...
if( !function_exists('wpestate_upgrade_user_membership') ):
    function wpestate_upgrade_user_membership($user_id,$pack_id,$type,$paypal_tax_id){
        ...
        $user_email=$user->user_email;
        wp_mail($user_email,
                sprintf(esc_html__( '[%s] Membership Activated','wpestate'), get_option('blogname')),
                $message,
                $headers);
...
if( !function_exists('wpestate_listing_set_to_expire') ): 
    function wpestate_listing_set_to_expire($post_id){
        ...
        wp_mail($user_email,
        sprintf(esc_html__( '[%s] Free Listing expired','wpestate'), get_option('blogname')),
        $message,
        $headers);
    }
endif;
