            ...
            if($prop_selection->have_posts()){ 
                ...
                while ($prop_selection->have_posts()): $prop_selection->the_post(); 
                    ...
                    if($user_email!='' && $mail_content!=''){
                        ...
                        @wp_mail(
                                $user_email,
                                esc_html__( 'Matching Submissions on ','wpestate').$_SERVER['HTTP_HOST'],
                                $mail_content,
                                $headers
                        );  
                    }
...
if( !function_exists('wpestate_compose_send_email') ): 
    function wpestate_compose_send_email($args){
        $mail_content=''; 
        $mail_content .= esc_html__( 'Hello,','wpestate')."\r\n".esc_html__( 'A new submission matching your chosen criteria has been published at ', 'wpestate').$_SERVER['HTTP_HOST']." \r\n ";
        $mail_content .=esc_html__( 'These are the new submissions:','wpestate')." \r\n ";
        $arguments = wpestate_objectToArray( json_decode($args) );
        ...
        if($prop_selection->have_posts()){ 
            ...
            while ($prop_selection->have_posts()): $prop_selection->the_post(); 
                $post_id=get_the_id();
                $mail_content .= esc_url ( get_permalink($post_id) )."\r\n";
            endwhile;
