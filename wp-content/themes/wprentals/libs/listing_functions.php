    ...
    function estate_listing_price($post_id){
        ...
        if($price_per_guest_from_one!=1){
            ...
            if ($price_per_weekeend!=0){
                $return_string.='<div class="listing_detail col-md-6"><span class="item_head">'.esc_html__( 'Price per weekend (Saturday & Sunday)','wpestate').':</span> ' . $price_per_weekeend_show . '</div>'; 
            }
            
            if ($extra_price_per_guest!=0){
                $return_string.='<div class="listing_detail col-md-6"><span class="item_head">'.esc_html__( 'Extra Price per guest','wpestate').':</span> ' . $extra_price_per_guest_show . '</div>'; 
            }
        }else{
            if ($extra_price_per_guest!=0){
                $return_string.='<div class="listing_detail col-md-6"><span class="item_head">'.esc_html__( 'Price per guest','wpestate').':</span> ' . $extra_price_per_guest_show . '</div>'; 
            }
        }
        
        if ($cleaning_fee != 0){
            $return_string.='<div class="listing_detail col-md-6"><span class="item_head">'.esc_html__( 'Cleaning Fee','wpestate').':</span> ' . $cleaning_fee_show ;
        ...
        if ($city_fee != 0){
            $return_string.='<div class="listing_detail col-md-6"><span class="item_head">'.esc_html__( 'City Tax Fee','wpestate').':</span> ' . $city_fee_show; 
        ...
        if ($min_days_booking!=0){
            $return_string.='<div class="listing_detail col-md-6"><span class="item_head">'.esc_html__( 'Minimum no of nights','wpestate').':</span> ' . $min_days_booking . '</div>'; 
        }
        
        if($overload_guest!=0){
            $return_string.='<div class="listing_detail col-md-6"><span class="item_head">'.esc_html__( 'Allow more guests than the capacity: yes','wpestate').'</span></div>'; 
        }
        
       
       
        if ($checkin_change_over!=0){
            $return_string.='<div class="listing_detail col-md-6"><span class="item_head">'.esc_html__( 'Booking starts only on','wpestate').':</span> ' . $week_days[$checkin_change_over ]. '</div>'; 
        }
        
        if ($checkin_checkout_change_over!=0){
            $return_string.='<div class="listing_detail col-md-6"><span class="item_head">'.esc_html__( 'Booking starts/ends only on','wpestate').':</span> ' .$week_days[$checkin_checkout_change_over] . '</div>'; 
        }
...
if( !function_exists('wpestate_show_custom_details') ):
    function wpestate_show_custom_details($edit_id,$is_dash=0){
        ...
        $price_array    =   get_post_meta($edit_id, 'custom_price'.$edit_id,true );
        ...
        if(is_array($mega)){
            ...
            foreach ($mega as $day=>$data_day){          
                ...
                if ( !array_key_exists ($tomorrrow_date,$mega) ){ // non consecutive days
                    ...
                    if( $data_day['period_min_days_booking']                !=  $flag_min_days || 
                        $data_day['period_extra_price_per_guest']           !=  $flag_guest || 
                        $data_day['period_price_per_weekeend']              !=  $flag_price_week || 
                        $data_day['period_checkin_change_over']             !=  $flag_change_over ||  
                        $data_day['period_checkin_checkout_change_over']    !=  $flag_checkout_over){
...
if( !function_exists('estate_listing_address') ):
    function estate_listing_address($post_id){
        ...
        if ($property_address != ''){
            $return_string.='<div class="listing_detail col-md-6"><span class="item_head">'.esc_html__( 'Address','wpestate').':</span> ' . $property_address . '</div>'; 
        }
        if ($property_city != ''){
            $return_string.= '<div class="listing_detail col-md-6"><span class="item_head">'.esc_html__( 'City','wpestate').':</span> ' .$property_city. '</div>';  
        }  
        if ($property_area != ''){
            $return_string.= '<div class="listing_detail col-md-6"><span class="item_head">'.esc_html__( 'Area','wpestate').':</span> ' .$property_area. '</div>';
        }    
        if ($property_county != ''){
            $return_string.= '<div class="listing_detail col-md-6"><span class="item_head">'.esc_html__( 'County','wpestate').':</span> ' . $property_county . '</div>'; 
        }
        if ($property_state != ''){
            $return_string.= '<div class="listing_detail col-md-6"><span class="item_head">'.esc_html__( 'State','wpestate').':</span> ' . $property_state . '</div>'; 
        }
        if ($property_zip != ''){
            $return_string.= '<div class="listing_detail col-md-6"><span class="item_head">'.esc_html__( 'Zip','wpestate').':</span> ' . $property_zip . '</div>';
        }  
        if ($property_country != '') {
            $return_string.= '<div class="listing_detail col-md-6"><span class="item_head">'.esc_html__( 'Country','wpestate').':</span> ' . $property_country . '</div>'; 
        } 
...
if( !function_exists('estate_listing_details') ):
    function estate_listing_details($post_id){
        ...
        $property_status=esc_html(get_post_meta($post_id, 'property_status', true));

        $return_string='';


        if ($property_status != ''){
            $return_string.= '<div class="listing_detail col-md-6"><span class="item_head">'.esc_html__( 'Property Status','wpestate').':</span> ' . $property_status . '</div>';
        }    
        if ($property_size != ''){
