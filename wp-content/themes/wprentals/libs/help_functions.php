...
if( !function_exists('wpestate_booking_price')):
    function wpestate_booking_price($curent_guest_no,$invoice_id, $property_id, $from_date, $to_date){
        ...
        $mega                       =   get_post_meta($property_id, 'mega_details'.$property_id,true );
        ...
        if( $invoice_id == 0){
            ...
            $week_price         =   intval(get_post_meta($property_id, 'property_price_per_week', true));
            $month_price        =   intval(get_post_meta($property_id, 'property_price_per_month', true));
            ...
            $price_per_day      =   intval(get_post_meta($invoice_id, 'default_price', true));
            $week_price         =   intval(get_post_meta($invoice_id, 'week_price', true));
            $month_price        =   intval(get_post_meta($invoice_id, 'month_price', true));
        ...
        if($price_per_guest_from_one == 0 ) {
            ...
            $total_price     =   $total_price + $count_days * $curent_guest_no * $extra_price_per_guest;
            $inter_price     =   $inter_price + $count_days * $curent_guest_no * $extra_price_per_guest;
...
if( !function_exists('wpestate_email_to_admin') ):
    function wpestate_email_to_admin($onlyfeatured){  
        ...
        if($onlyfeatured==1){
            $title= sprintf(esc_html__( '[%s] New Feature Upgrade ','wpestate'), get_option('blogname'));
            $message .= sprintf( esc_html__( "You have a new featured submission on  %s! You should go check it out.",'wpestate'), get_option('blogname')) . "\r\n\r\n";

        }else{
             $title= sprintf(esc_html__( '[%s] New Paid Submission','wpestate'), get_option('blogname'));
             $message .= sprintf( esc_html__( "You have a new paid submission on  %s! You should go check it out.",'wpestate'), get_option('blogname')) . "\r\n\r\n";
...
if( !function_exists('wpestate_send_booking_email') ):
    function wpestate_send_booking_email($email_type,$receiver_email,$content=''){
        ...
        if ($email_type == 'bookingconfirmeduser'){
               $subject = sprintf (esc_html__( 'Booking Confirmed on %s. ','wpestate'),get_site_url()); 
               $message = sprintf ( esc_html__( 'Your booking made on %s was confirmed! You can see all your reservations by logging in your account and visiting My Reservations page. ','wpestate'),get_site_url());
        }
        if ($email_type == 'bookingconfirmed'){
               $subject = sprintf( esc_html__( 'Booking Confirmed on %s.','wpestate'),get_site_url() ); 
               $message = sprintf( esc_html__( 'Somebody confirmed a booking on %s! You should go and check it out!Please remember that the confirmation is made based on the payment confirmation of a non-refundable fee of the total invoice cost, processed through %s and sent to website administrator. ','wpestate'),get_site_url(),get_site_url());
        }else if ($email_type == 'bookingconfirmed_nodeposit'){
               $subject = sprintf(esc_html__( 'Booking Confirmed on %s.','wpestate'),get_site_url());
               $message = sprintf(esc_html__( 'You confirmed a booking on %s! The booking was confirmed with no deposit! ','wpestate'),get_site_url());
        }
        else if ($email_type == 'inbox'){
                $subject = sprintf(esc_html__( 'New Message on %s.','wpestate'),get_site_url());
                $message = sprintf(esc_html__( 'You have a new message on %s! You should go and check it out! ','wpestate'),get_site_url());
                if ($content!=''){
                     $message .=' '.esc_html__('The message is: ','wpestate').$content;
                }
        }
        else if ($email_type == 'newbook'){
               $property_id= intval($content);
               $subject = sprintf(esc_html__( 'New Booking Request on %s.','wpestate'),get_site_url());
               $message = sprintf(esc_html__( 'You have received a new booking request on %s!  Go to your account in Bookings page to see the request, issue the invoice or reject it! ','wpestate'),get_site_url());
               $message .= esc_html__('The property is: ','wpestate').get_permalink($property_id).' .';
               
        }
        else if ($email_type == 'mynewbook'){
                $property_id= intval($content);
                $subject = sprintf(esc_html__( 'You booked a period on %s.','wpestate'),get_site_url());
                $message = sprintf(esc_html__( 'You have booked a period for your own listing on %s!  The reservation will appear in your account, under My Bookings. ','wpestate'),get_site_url());
                $message .= esc_html__('The property is: ','wpestate').get_permalink($property_id).' .';
               
        }
        else if ($email_type == 'newinvoice'){
               $subject = sprintf(esc_html__( 'New Invoice on %s.','wpestate'),get_site_url()); 
               $message = sprintf(esc_html__( 'An invoice was generated for your booking request on %s!  A deposit will be required for booking to be confirmed. For more details check out your account, "My Reservations" page.','wpestate'),get_site_url());
        }
        else if ($email_type == 'deletebooking'){
               $subject = sprintf(esc_html__( 'Booking Request Rejected on %s.','wpestate'),get_site_url());
               $message = sprintf(esc_html__( 'One of your booking requests sent on %s was rejected by the owner. The rejected reservation is automatically removed from your account. ' ,'wpestate'),get_site_url());
        }
        else if ($email_type == 'deletebookinguser'){
               $subject = sprintf(esc_html__( 'Booking Request Cancelled on %s.','wpestate'),get_site_url()); 
               $message = sprintf(esc_html__( 'One of the unconfirmed booking requests you received on %s  was cancelled! The request is automatically deleted from your account! ','wpestate'),get_site_url());
        }else if ($email_type == 'deletebookingconfirmed'){
               $subject = sprintf(esc_html__( 'Booking Period Cancelled on %s.','wpestate'),get_site_url()); 
               $message = sprintf(esc_html__( 'One of your confirmed bookings on %s  was cancelled by property owner. ','wpestate'),get_site_url());
        }
...
if( !function_exists('wpestate_get_guest_dropdown') ):
    function wpestate_get_guest_dropdown($with_any=''){
        $select_area_list='';
        if($with_any==''){
            $select_area_list.='<li role="presentation" data-value="0">'.esc_html__( 'any','wpestate').'</li>';
        }
        $select_area_list .=   '<li role="presentation" data-value="1">1 '.esc_html__( 'guest','wpestate').'</li>';
        for($i=2;$i<15;$i++){
            $select_area_list .=   '<li role="presentation" data-value="'. $i.'">'. $i.' '.esc_html__( 'guests','wpestate').'</li>';
        }
...
if( !function_exists('wpestate_check_booking_valability') ):
    function wpestate_check_booking_valability($book_from,$book_to,$listing_id){
        ...
        $to_date_unix       =   $to_date->getTimestamp();
         ...
         //print 'start on '. $from_date_unix.' / '.$to_date_unix;
        //echo 'starting with '.$listing_id;
        //echo 'first '.$from_date->format('Y-m-d ') .' / '. $to_date->format('Y-m-d ').' /////</br>';
        ...
        while ($from_date_unix < $to_date_unix){
          ...
          //echo $from_date->format('Y-m-d ') .' / '. $to_date->format('Y-m-d ').' /////</br>';
            $from_date_unix =   $from_date->getTimestamp();
            if( array_key_exists($from_date_unix,$reservation_array ) ){
                return false;
            }
