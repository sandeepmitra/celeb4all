...
if( 'POST' == $_SERVER['REQUEST_METHOD'] ) {
    ...
    if ( $paid_submission_status!='membership' || ( $paid_submission_status== 'membership' || wpestate_get_current_user_listings($userID) > 0)  ){ // if user can submit        
        ...
        if($post_id) {
            ...
            $message .= sprintf( esc_html__( "A user has submited a new listing on  %s! You should go check it out. This is the lsiting title: %s",'wpestate'), get_option('blogname'),$submit_title) . "\r\n\r\n";
