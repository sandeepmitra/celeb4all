    ...
    if( $strNewStatus === 'publish' ) {
            ...
            if( $user->roles[0]=='subscriber'){
                ...
                if($strOldStatus=='pending'){
                    ...
                    $message .= sprintf( esc_html__( "Your listing %s was approved on  ",'wpestate'), get_the_title ($post->ID) ) .get_option('blogname'). ".\r\n\r\n";

                    $message .= sprintf( esc_html__( 'The listing is: %s','wpestate'), get_permalink($post->ID) ) . "\r\n";
                    ...
                    wp_mail(
                            ...
                            sprintf( esc_html__( '[%s] Your listing was approved','wpestate'), get_option('blogname') ),
...
function inforain_redirect_role_fn() {
    wp_redirect( home_url( 'user-profile' ) );
}


function add_more_field_video() {
    echo <<<'EOT'
    <script type="text/javascript">
        ...
        jQuery("#add_more_video_field").click(function(e){ //on add input button click
            e.preventDefault();
            if(x < max_fields){ //max input box allowed
                x++; //text box increment
                jQuery(wrapper).append('<div><input type="text" name="video_youtube[]" placeholder="https://www.youtube.com/watch?v=JlimhpwZDeE"/><a href="#" class="remove_field">Remove &minus;</a></div>'); //add input box
            }
        });
...
function custom_role_user() {
    add_role('celleb', 'Celebrity');
}
...
function themeslug_enqueue_style() {
    wp_enqueue_style( 'datetimepickercss', get_template_directory_uri() . '/css/dateTimePicker.css', false ); 
}

function themeslug_enqueue_script() {
    wp_enqueue_script( 'datetimepickerjs', get_template_directory_uri() . '/js/dateTimePicker.js', false );
}
   ...
   jQuery(document).ready(function()
    {
        ...
        jQuery('#show-next-month').calendar(
      {
        num_next_month: 1,
        num_prev_month: 1,
        unavailable: ['2017-01-09', '2017-01-10', '2017-01-11', '2017-01-12', '2017-02-24', '2017-02-25']
      });
    });
    </script>
...
function edit_profile() {
 ...
 ?>
    ...
    function disabled_previous_selected_dates(date) {
        jQuery("#datepicker_end_date").datepicker('option', 'minDate', date);
    }

    function disabled_perticular_dates(date){
        var array = ["2017-01-09","2017-01-10","2017-01-12"]
        var string = jQuery.datepicker.formatDate('yy-mm-dd', date);
        return [ array.indexOf(string) == -1 ];
    }

  jQuery( function() {
    jQuery( "#datepicker" ).datepicker({ dateFormat: 'yy-mm-dd', minDate: 0, beforeShowDay: disabled_perticular_dates() });
    jQuery( "#datepicker_end_date" ).datepicker({ dateFormat: 'yy-mm-dd', minDate: 0, beforeShowDay: disabled_perticular_dates() });
  });
  //----------------------------------------------------------
   jQuery(document).ready(function(){
        ...
        jQuery("#span_edit_heading"). click(function(){
            jQuery("#edit_heading_val").show();
            jQuery("#edit_heading_val").focus();
            jQuery("#span_edit_heading").hide(function(){
                jQuery("#span_save_heading").show();
            });
        ...
        //save the value
        jQuery("#span_save_heading").click(function(){
            var loginName = jQuery("#edit_heading_val").val();
            jQuery.post("<?php echo get_template_directory_uri(); ?>/ajax_save_profile.php", {loginName : loginName, q : '1'}, function(data){
                console.log(data);
                jQuery("#edit_heading_val").hide(function(){
                    jQuery(".entry-title-agent").html(data);
                    jQuery("#span_save_heading").hide();
                });
            });
        });
        ...
        jQuery("#span_edit_live"). click(function(){
            jQuery("#edit_live_val").show();
            jQuery("#edit_live_val").focus();
            jQuery("#span_edit_live").hide();
                jQuery("#span_save_live").show();


        });

        //save the value
        jQuery("#span_save_live").click(function(){
            var loginName = jQuery("#edit_live_val").val();
            jQuery.post("<?php echo get_template_directory_uri(); ?>/ajax_save_profile.php", {loginName : loginName, q : '2'}, function(data){
                console.log(data);
                jQuery("#edit_live_val").hide(function(){
                    jQuery("#livein").html(data);
                    jQuery("#span_save_live").hide();
                });
            });
        });
...
add_action( 'inforain_redirect_role', 'inforain_redirect_role_fn' );
add_action( 'wp_footer', 'add_more_field_video' );
add_action( 'admin_init', 'custom_role_user' );
//add_action( 'template_redirect', 'redirect_non_logged_users_to_specific_page' );
add_action( 'wp_enqueue_scripts', 'themeslug_enqueue_style' );
add_action( 'wp_enqueue_scripts', 'themeslug_enqueue_script' );
add_action( 'wp_footer','slider_effect' );
add_action( 'wp_footer','edit_profile' );
?>